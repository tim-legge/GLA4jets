...
class GLABlock(nn.Module):
    ...
    def forward(self, x, x_cls=None, attention_mask=None, **kwargs):
        # Self-GLA with residual connection
        if x_cls is not None:
            with torch.no_grad():
                padding_mask = torch.cat((torch.zeros_like(attention_mask[:,:1]), attention_mask), dim=1)
            residual = x_cls
            u = torch.cat((x_cls,x), dim=0) # append class token onto static x tokens (seq_len+1, batch, embed_dim)
            u = self.attn_norm(u)
            attn_out, _, _ = self.gla(u, attention_mask=attention_mask, **kwargs)
...
class InputEmbedding(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x = x.permute(2, 0, 1).contiguous()  # (L, batch, input_dim)
        ...
        return x # (seq_L, batch, embed_dim)
...
class GLATransformer(nn.Module):
    ...
    def __init__(
        ...
        **gla_kwargs
        ...
        # Stack of class GLA blocks - output single classification token
        self.cls_blocks = nn.ModuleList([GLABlock(
            embed_dim=embed_dim,
                num_heads=num_heads,
                expand_ratio=expand_ratio,
                dropout=dropout,
    ...
    def forward(self, x, mask=None):
        ...
        # class token
        cls_token = self.cls_token.expand(1,x.size(1),-1) # (1, N, C)
        for block in self.cls_blocks:
            cls_token = block(x, x_cls=cls_token, attention_mask=padding_mask)
        
        # squeeze final class token to (N, C) and norm
        x = self.final_norm(cls_token).squeeze(0)
        
        # Classification
        logits = self.classifier(x)
        #output = torch.softmax(logits, dim=-1)
        print(logits.shape)
        return logits # (N, num_labels)
